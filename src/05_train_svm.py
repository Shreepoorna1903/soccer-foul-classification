"""
05_train_svm.py
Train SVM with RBF kernel + 5-fold stratified cross-validation
on handcrafted features for both binary and multiclass tasks.

Usage:
    python 05_train_svm.py
"""
...
def run_task(task, X_train, y_train, X_test, y_test, label_names):
    ...
    # ── CV results summary ──
    print(f"\n  CV Results:")
    print(f"  {'C':<8} {'gamma':<10} {'Mean BA':>10} {'Std BA':>10}")
    print(f"  {'-'*42}")
    ...
    for i in range(len(results["params"])):
        ...
        marker = " <-- best" if p == grid.best_params_ else ""
        print(f"  {p['C']:<8} {str(p['gamma']):<10} {mean:>10.4f} {std:>10.4f}{marker}")
    ...
    ax.set_title(f"SVM (RBF) — {task}\nTest Balanced Acc: {test_ba:.4f}")
    plt.tight_layout()
    fig_path = RESULTS_DIR / f"svm_{task}_confusion.png"
    fig.savefig(fig_path, dpi=150)
    plt.close()
    print(f"  Confusion matrix saved to {fig_path}")

    # ── Save model + scaler ──
    model_path = MODELS_DIR / f"svm_{task}.pkl"
